...
- ✅ Lock для `/mySites`: параллельные запуски тихо пропускаются (`exit 0`)
- ✅ 84 теста с assertions
...
- TF/CF: уведомляем о росте или падении на 2 и более относительно предыдущего
  замера. Старое значение `cf_tf = 1` в `[metrics]` также ограничивается минимумом 2.
- Достижение цели TF/CF (переход с значения ниже 30 на 30 или выше) уведомляется
  всегда, даже при шаге 29 → 30 и при увеличенном пороге в конфиге.
- Изменения на 1 продолжают сохраняться в БД; они не накапливаются для отчёта.
  Первое появление метрики без предыдущего значения уведомляется как раньше.
- После домена из `ggl_top` в отчёте метрик ставится 👑. Список из 82 доменов
  скопирован из `ddl.domain WHERE ggl_top = 1` 21.09.2026 в `ggl_top.py`.
  Обновлять список нужно вручную; автоматической синхронизации нет.
...
### Полная локальная выгрузка площадок

```bash
venv/bin/python export_sites.py --output-dir outputs/sites-export
```

Экспортёр работает только на чтение и не подключается к MySQL. Он сохраняет
нормализованный CSV для фильтрации и JSON с исходным HTML, ссылками, скрытыми
полями, подсказками и карточками причин отклонения всех площадок из `/mySites`.
...
| Частые "Skipping run because mySites lock is active" | Проверить долгий/зависший запуск; lock живёт в `/tmp/gogetlinks_mysites.lock` |
...
**Статус:** ✅ v1.2.2 — mySites lock + статус-уведомления + split-schedule + 84 теста
**Версия:** 1.2.2
